...
class Meal:
	...
	def __str__(self):
		...
		for dish in self.items:
			macros = ["Serving Size: {}".format(SERVING_SIZES[dish].strip())]
			macros += ["{}: {}{}".format(nutrient, int(self.items[dish].nutrition[nutrient]),
				TARGET_NUTRIENTS_UNITS[nutrient]) 
				for nutrient in TARGET_NUTRIENTS]
			return_strings.append("{}. {} ({})".format(i,dish,", ".join(macros)))
			i+=1
		return_strings.append('')
		return_strings+=["Total {}: {}{}".format(nutrient,int(self.totals[nutrient]),
			TARGET_NUTRIENTS_UNITS[nutrient]) for nutrient in TARGET_NUTRIENTS]
		return_strings.append('')
		return_strings+=["{} surplus/deficit: {}{}".format(nutrient,
			int(self.totals[nutrient]-self.boundaries[nutrient]),
			TARGET_NUTRIENTS_UNITS[nutrient]) for nutrient in TARGET_NUTRIENTS]
		return_strings.append('')
		return "\n".join(return_strings)
